...
if cfg.do_save_fig
    ...
    fname2save = fullfile(cfg.path.project, cfg.path.data_prep, ...
        sprintf('%s_preproc_notchdata_scroll.jpg',cfg.folder_name));
...
bad_chan_idx = find(sum(bad_chan_table,1) >= cfg.prep.asr.niter_bad_chan);  % bad channel identified in more than n iterations
bad_sample_idx = find(sum(bad_sample_table,1) >= cfg.prep.asr.niter_bad_chan);  % bad sample identified in more than n iterations
...
if cfg.do_save_fig
    fname2save = fullfile(cfg.path.project, cfg.path.data_prep, ...
        sprintf('%s_ASR_result.jpg',cfg.folder_name));
    print(gcf,fname2save, '-djpeg','-noui');
end
...
% % select bad_channel only based on ASR
% bad_chan_label = {};
% counter = 1;
...
% = = = = = = = = = = = = = = = = = == 
% identify BAD CHANNELS based on mean_correlation, variance and HURST exponent
% as in FASTER toolbox (with a z_threshold = 2.5 
[ bad_chan_zthresh3, chan_properties, z_score ] = BADCHAN_detection_continuous(eeg_notch, 3);
[ bad_chan_zthresh2, chan_properties, z_score ] = BADCHAN_detection_continuous(eeg_notch, 2.5);
length(bad_chan_zthresh2)

% = = = = = = = = = = = = = = = = = == 
% COMPARE and MERGE the results of the 2 methods:
bad_chan_ASR = sum(bad_chan_table,1);

bad_chan_FAST = zeros([1,size(chan_properties,1)]);
bad_chan_FAST(bad_chan_zthresh2) = 1;
...
if cfg.do_save_fig
    ...
    %     sprintf('%s_%s_preproc_badSamplesChannels.jpg',subid,dataType));
    fname2save = fullfile(cfg.path.project, cfg.path.data_prep, ...
        sprintf('%s_preproc_badSamplesChannels.jpg',cfg.folder_name));
    print(gcf,fname2save, '-djpeg','-noui');
end

% STORE this information into the cfg.prep structure:
cfg.prep.bad_chan_table = bad_chan_table;
cfg.prep.bad_chan_label = bad_chan_label;
cfg.prep.bad_sample_tableASR = bad_sample_table;
cfg.prep.bad_sample_perc = bad_sample_perc;
cfg.prep.bad_sample_tableASR2 = bad_sample_ASR2_table;
...
if cfg.do_save_fig
    ...
    fname2save = fullfile(cfg.path.project, cfg.path.data_prep, ...
        sprintf('%s_preproc_IC_brain_probabilities.jpg',cfg.folder_name));
    print(gcf,fname2save, '-djpeg','-noui');
end
...
if cfg.do_save_fig
    fname2save = fullfile(cfg.path.project, cfg.path.data_prep, ...
        sprintf('%s_preproc_ica_1_28.jpg',cfg.folder_name));
    print(gcf,fname2save, '-djpeg','-noui');
end
...
if cfg.do_save_fig
    fname2save = fullfile(cfg.path.project, cfg.path.data_prep, ...
        sprintf('%s_preproc_psd.jpg',cfg.folder_name));
    print(gcf,fname2save, '-djpeg','-noui');
end
...
%% SAVE
% ===============================================
% KEEP data at 4 prep stages: 
% after NOTCH; after ASR repair; after ICA rejection
eeg_prep.eeg_notch = eeg_notch;
%eeg_prep.eeg_afterASR = eeg_cleanraw_iter;
eeg_prep.eeg_afterASR = eeg_cleanraw;
eeg_prep.eeg_beforeICA = eeg_cleanraw_avgref_ICA;
eeg_prep.eeg_afterICA = eeg_cleanraw_avgref_nobadICA;
eeg_prep.cfg = cfg;
...
% help vis_artifacts
%vis_artifacts(eeg_cleanraw, eeg_notch, 'WindowLength', 400)
...
if cfg.do_save_fig
    fname2save = fullfile(cfg.path.project, cfg.path.data_prep, ...
        sprintf('%s_nobadICA_comparison.jpg',cfg.folder_name));
    print(gcf,fname2save, '-djpeg','-noui');
end
...
cd(fullfile(cfg.path.project, cfg.path.data_prep))
if n_icacomponents2keep~=0
    %fname2save = fullfile(cfg.preproc_data_dir, subid, dataType, ...
