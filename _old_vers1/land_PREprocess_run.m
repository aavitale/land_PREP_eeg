...
% 2° step: DETECT BAD CHANNELS (and samples) with ASR
% 4° step: detect BAD CHAN with FASTER metrics
% 5° step: BAD CHAN intersect and interpolation 

% 3° step: detect remaining BAD sample (with ASR)

% 1° step: REPAIR the timeseries without removing any channel or sample
...
[eeg_cleanraw_ASR2, HP, BUR, bad_chan_cleanraw] = clean_artifacts(eeg_cleanraw, ...
     ...
     'Highpass',cfg.prep.asr.highpass, ...
     'BurstRejection','on');
bad_sample_ASR2 =  ~eeg_cleanraw_ASR.etc.clean_sample_mask;
...
cfg.prep.bad_sample_ASR2 = bad_sample_ASR2;
...
xlabel('iteration');
ylabel('CHANNEL')
title([ '% Bad Channels ASR = ' num2str(length(bad_chan_ASR)/size(bad_chan_table,2)*100)]);
...
topoplot(bad_chan_FASTidx, eeg_notch.chanlocs, ...
    ...
    'maplimits', [ -5 20 ]) ;
title('Bad channel FASTer')
colormap(gca,'gray')
%x_lim = get(gca, 'Xlim')
%text(x_lim(1)*1.5,0, bad_chan_label, 'fontweight','bold') %,'FontSize',14)


figure(3); subplot(236)
...
title(sprintf('% Bad Samples = %f', mean(bad_sample_ASRperc)));
